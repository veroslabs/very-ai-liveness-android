...
```gradle
dependencies {
    implementation 'org.very:liveness:1.0.56'
}
```
...
| File | Purpose |
|---|---|
| `liveness-1.0.56.aar` | Main SDK — `org.very:liveness` |
| `sdk-native-bundle-1.0.56.aar` | Optional bundled `.so` companion (see *Asset loading* below) |
...
```gradle
dependencies {
    implementation fileTree(dir: 'libs', include: ['liveness-1.0.56.aar'])
    // Optional — opt into bundled mode (see Asset loading)
    implementation fileTree(dir: 'libs', include: ['sdk-native-bundle-1.0.56.aar'])
...
dependencies {
    implementation 'org.very:liveness:1.0.56'
    implementation 'org.very:sdk-native-bundle:1.0.56'
}
```
...
The demo's `app/build.gradle` pulls `org.very:liveness:1.0.56` from
