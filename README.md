...
```gradle
dependencies {
    implementation 'org.very:liveness:1.0.64'
}
```
...
dependencies {
    implementation 'org.very:liveness:1.0.64'
    implementation 'org.very:sdk-native-bundle:1.0.64'
}
```
...
| File | Purpose |
|---|---|
| `liveness-1.0.64.aar` | Main SDK — `org.very:liveness` |
| `sdk-native-bundle-1.0.64.aar` | Optional bundled `.so` companion (see *Install — Gradle* above) |
...
```gradle
dependencies {
    implementation fileTree(dir: 'libs', include: ['liveness-1.0.64.aar'])
    // Optional — opt into bundled mode
    implementation fileTree(dir: 'libs', include: ['sdk-native-bundle-1.0.64.aar'])
...
The demo's `app/build.gradle` pulls `org.very:liveness:1.0.64` from
